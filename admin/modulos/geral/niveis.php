          ...
          <? if($acao == "insert" || $acao == "edit"){?>
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              ...
              <form id="form-geral" class="form-horizontal form-label-left" method="post" action="<?=$action_form; ?>" enctype="multipart/form-data">     
                ...
                <!--repete -->
                <? $permissoes_atuais_niveis = explode(",", $linha_edit['permissoes']); ?>
                <link href="<?=URL;?>admin/vendors/multiple-select-master/multiple-select-admin.css" rel="stylesheet" />
                <div class="item form-group">
                  <label class="control-label col-md-3 col-sm-3 col-xs-12" for="permissoes">
                    Permissões *                                  
                  </label>
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <select name="permissoes[]" id="permissoes" multiple="multiple" required>
                      <? foreach ($row_cats_niveis as $cats_niveis) { ?>
                        <option value="<?=$cats_niveis['categoria']; ?>" <? if(in_array($cats_niveis['categoria'], $permissoes_atuais_niveis)){ ?> selected <? } ?>><?=$cats_niveis['titulo']; ?></option> 
                      <? } ?>
                    </select>  
                    <label for="permissoes" class="error"></label>                                   
                  </div>
                </div>  
                <script src="<?=URL;?>admin/vendors/multiple-select-master/multiple-select.js"></script>
                <script>            
                  $('#permissoes').multipleSelect({
                    filter: true,
                    placeholder: "Selecione",
                    selectAll: true,
                    minimumCountSelected: 5,                    
                    allSelected: 'Todas as permissões',
                    countSelected: '# de % selecionadas',
                    noMatchesFound: 'Permissão não encontrada',
                    ...
                    <tbody>
                      ...
                      <? foreach ($resultado as $linha) { ?>
                        ...
                        <tr>
                          ...
                          <!--item -->
                          <td data-label="Nível"><?=$linha['titulo']; ?></td>
                          <!--item -->
