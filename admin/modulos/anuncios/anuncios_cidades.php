            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              ...
              <form id="form-geral" class="form-horizontal form-label-left" method="post" action="<?=$action_form; ?>" enctype="multipart/form-data">
                ...
                <div class="item form-group">
                  <label class="control-label col-md-3 col-sm-3 col-xs-12" for="estado_id">
                    Estado *
                  </label>
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <select name="estado_id" id="estado_id" class="form-control col-md-7 col-xs-12" required>
                      <option value="" hidden>Selecione</option>
                      <? foreach ($estados as $uf) { ?>
                        <option value="<?=$uf['id']?>" <?=Tools::selected($uf['id'], $linha_edit['estado_id']); ?>><?=$uf['titulo']?></option>
                      <? } ?>
                    </select>
                  </div>
                </div>
                  ...
                  <form method="post" action="<?=$acao_filtros?>">
                    <div class="filtros-itens">
                      ...
                      <div class="campo-filtro form-group">
                        <label for="filtro_titulo" class="control-label">Cidade:</label>
                      ...
                      <!-- Item filtro -->
                      <div class="campo-filtro form-group">
                        <label for="filtro_estado" class="control-label">Estado:</label>
                        <select name="filtro_estado" id="filtro_estado" class="form-control">
                          <option value="">Todos</option>
                          <? foreach ($estados as $uf) { ?>
                            <option value="<?=$uf['id']?>" <?=Tools::selected($_SESSION[$pag_include]['filtros']['filtro_estado'], $uf['id'])?>><?=$uf['titulo']?></option>
                          <? } ?>
                        </select>                        
                      </div>
                      <!-- // Item filtro -->
                      ...
                      <div class="campo-filtro form-group">
                        ...
                        <select name="filtro_status" id="filtro_status" class="form-control">
                          <option value="">Todos</option>
                          <option value="1" <?=Tools::selected($_SESSION[$pag_include]['filtros']['filtro_status'], "1")?>>Ativas</option>
                          <option value="0" <?=Tools::selected($_SESSION[$pag_include]['filtros']['filtro_status'], "0")?>>Inativas</option>
                        </select>                        
                      </div>
                      <!-- // Item filtro -->
                  ...
                  <table class="table table_responsive">
                    ...
                    <tbody>
                      ...
                      <? foreach ($resultado as $linha) { ?>
                        ...
                        $current_url_bairros = $_GET['p'] != '' ? 
                        "".URL."admin/".$pasta_modulo."/anuncios_bairros/".TOKEN."/view/0/".$linha['id']."?p2=".$_GET['p'] : 
                        "".URL."admin/".$pasta_modulo."/anuncios_bairros/".TOKEN."/view/0/".$linha['id'];  
                        ?>
                        ...
                        <tr>
                          ...
                          <!--item -->
                          <td data-label="Cidade"><?=$linha['titulo']; ?></td>
                          <!--item --> 
                          ...
                          <!--item -->
                          <td data-label="Bairros">
                            <a class="btn btn-sm btn-default" href="<?=$current_url_bairros?>"><i class="fas fa-bars"></i> Bairros (<?=$linha['total_bairros']?>)</a>
                          </td>
                          <!--item -->
