...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar logo";
		break;
	case 'edit':
		$tit_pag_geral = "Editar logo";
		break;
	default:
		$tit_pag_geral = "Logos";
		break;
		...
		if ($verificaUnicos === true) {
			...
			if ($verificaImagens === true) {
				...
				if ($operacao) {
					...
					// UPLOAD DE IMAGEM
					if ($uploadArquivo) {	
						if ($_POST['local'] == 'icon_admin') {
							$redimensiona = true;
							$largura = 150;
							$altura = 150;
							$forma_redimensiona = 'crop'; // '', 'crop', 'auto'
            }
            if ($_POST['local'] == 'email') {
							$redimensiona = true;
							$largura = 150;
							$altura = 0;
