...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar acesso";
		break;
	case 'edit':
		$tit_pag_geral = "Editar acesso";
		break;
	default:
		$tit_pag_geral = "Níveis de acesso";
		break;
}
...
// CATEGORIAS
$row_cats_niveis = $acoes->SelectSQL("SELECT * FROM admin_categorias_niveis ORDER BY titulo ASC"); 
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "insert" && TOKEN == $token_confirma){
		...
		if ($verificaUnicos === true) {
			...
			if ($verificaImagens === true) {
				...
				$dados = array(
					'titulo' => $_POST['titulo'],
					'permissoes'=> "alertas,meus_dados,".implode(',', $_POST['permissoes']),
...
if($acao == "delete" && TOKEN == $token_confirma_del){
	...
	$operacao = $acoes->Delete($idDel, "WHERE id=$idDel", array(), $ordemExibicao, $removeDiretorio, $pathRemove);
