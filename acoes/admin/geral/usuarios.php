...
// CONFIGURAÇÕES
$tabela = "admin";
$filtro = "WHERE email <> 'contato@ellosdesign.com.br'";
...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar administrador";
		break;
	case 'edit':
		$tit_pag_geral = "Editar administrador";
		break;
	default:
		$tit_pag_geral = "Administradores";
		break;
}
...
// NÍVEIS DE ACESSO
$row_niveis = $acoes->SelectSQL("SELECT * FROM admin_niveis ORDER BY titulo ASC");
...
if ($acao == "view") {	
	...
	// FILTRO 'NÍVEL'
	if (isset($_POST['filtro_nivel'])) {
	    if ($_POST['filtro_nivel'] != "") {
	        $_SESSION[$pag_include]['filtros']['filtro_nivel'] = addslashes($_POST['filtro_nivel']);
	        $filtro_nivel = " AND nivel = '".$_SESSION[$pag_include]['filtros']['filtro_nivel']."'";	
	        $filtro .= $filtro_nivel;       
	    }else{
	        unset($_SESSION[$pag_include]['filtros']['filtro_nivel']);
	        $filtro_nivel = "";
	    }
	} else if (isset($_SESSION[$pag_include]['filtros']['filtro_nivel'])) {
	    $filtro_nivel = " AND nivel = '".$_SESSION[$pag_include]['filtros']['filtro_nivel']."'";
	    $filtro .= $filtro_nivel;
	}
		...
		if ($verificaUnicos === true) {
			...
			if ($verificaImagens === true) {
				...
				$dados = array(
					...
					'chave' => Tools::geraHash("md5",$_POST['data_cad']),
					'token' => Tools::geraHash("sha1",$hoje),
					'nivel' => $_POST['nivel'],
