...
// Cidade
$cidade =  $acoes->SelectSingle("SELECT * FROM $tabelaCidades WHERE id = $id_enviado2");
if ($cidade == "") {
	Tools::redireciona("".URL."admin/".$pasta_modulo."/".$tabelaCidades."/".TOKEN."/view");
}

$filtro = "WHERE cidade_id = ".$cidade['id'];
switch ($acao_enviado) { 
	case 'insert':
		$tit_pag_geral = $cidade['titulo']." &rsaquo; Cadastrar bairro";
		break;
	case 'edit':
		$tit_pag_geral = $cidade['titulo']." &rsaquo; Editar bairro";
		break;
	default:
		$tit_pag_geral = $cidade['titulo']." &rsaquo; Bairros";
		break;
}
...
"".URL."admin/".$pasta_modulo."/anuncios_cidades/".TOKEN."/view?p=".$_GET['p2'] :
"".URL."admin/".$pasta_modulo."/anuncios_cidades/".TOKEN."/view";

// CONFIGURAÇÕES DE PAGINAÇÃO
$url_paginacao = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/view/0/".$cidade['id'];
if (isset($_GET['p']) && $_GET['p'] != '') {
	$current_url_view = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/view/0/".$cidade['id']."?p=".$_GET['p'];
	$current_url_insert = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/insert/0/".$cidade['id']."?p=".$_GET['p'];
	$current_url_delete = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/delete/0/".$cidade['id']."?p=".$_GET['p'];
} else {
	$current_url_view = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/view/0/".$cidade['id'];
	$current_url_insert = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/insert/0/".$cidade['id'];
	$current_url_delete = "".URL."admin/".$pasta_modulo."/".$pag_include."/".TOKEN."/delete/0/".$cidade['id'];
}	
...
if ($acao == "view") {	
  ...
  foreach ($resultado as $key => $value) {
    // Total bloqueados
    $totalbloqueados = $acoes->SelectTotalSQL("SELECT id FROM enderecos_bloqueados WHERE bairro_id=".$value['id']);
    $resultado[$key]['total_bloqueados'] = $totalbloqueados;
  }
