<?
// CONFIGURAÇÕES
$tabela = "anuncios_taxas";
$filtro = "WHERE id > 0";
$order = "ORDER BY uf ASC";
$paginacao = true;
$num_regs = 30;
$ordemExibicao = false;
$uploadArquivo = false;
...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar taxa";
		break;
	case 'edit':
		$tit_pag_geral = "Editar taxa";
		break;
	default:
		$tit_pag_geral = "Taxas de venda";
		break;
}
...
if($acao == "edit"){
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "edit" && TOKEN == $token_confirma){
        ...
        // DADOS
        $dados = array(
          'itbi' => $_POST['itbi'] != "" ? str_replace(",",".",$_POST['itbi']) : 0,
          'escritura' => $_POST['escritura'] != "" ? str_replace(",",".",$_POST['escritura']) : 0,
          'registro' => $_POST['registro'] != "" ? str_replace(",",".",$_POST['registro']) : 0,
        );
