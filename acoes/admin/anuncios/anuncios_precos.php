...
$filtro = "WHERE anuncio_id = ".$anuncio['id'];
switch ($acao_enviado) { 
	case 'insert':
		$tit_pag_geral = $anuncio['titulo']." &rsaquo; Cadastrar tarifa";
		break;
	case 'edit':
		$tit_pag_geral = $anuncio['titulo']." &rsaquo; Editar tarifa";
		break;
	default:
		$tit_pag_geral = $anuncio['titulo']." &rsaquo; Tarifas Diferenciadas";
		break;
}
...
"".URL."admin/".$pasta_modulo."/".$tabelaAnuncio."/".TOKEN."/view?p=".$_GET['p2'] :
"".URL."admin/".$pasta_modulo."/".$tabelaAnuncio."/".TOKEN."/view";
...
if ($acao == "view") {	
	...
	// ORDENS DE EXIBIÇÃO
	if ($ordemExibicao) {
		$ordens = $acoes->SelectSQL("SELECT ordem_exibicao FROM $tabela WHERE anuncio_id = ".$anuncio['id']." ORDER BY ordem_exibicao ASC");
	}
...
if($acao == "insert"){
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "insert" && TOKEN == $token_confirma){
		...
		if ($verificaUnicos === true) {
          ...
          'data_inicial' => Tools::formataDataBd($_POST['data_inicial']),
          'data_final' => Tools::formataDataBd($_POST['data_final']),
          'valor' => Tools::formataMoedaBd($_POST['valor'])
				);
				...
				if ($operacao) {
					...
					// Ordem de Exibição
					if ($ordemExibicao) {
						$prox_valor = $acoes->SelectSingle("SELECT MAX(ordem_exibicao) AS maior_valor FROM $tabela WHERE anuncio_id=".$anuncio['id']." LIMIT 1")['maior_valor'];	
...
if($acao == "delete" && TOKEN == $token_confirma_del){
	...
	if ($ordemExibicao) {
		$result_item_del = $conexao->prepare("SELECT anuncio_id, ordem_exibicao FROM $tabela WHERE id = $idDel");
	...
	if ($operacao) {
		...
		if ($ordemExibicao) {				
			$update_itens = $conexao->prepare("UPDATE $tabela SET ordem_exibicao = ordem_exibicao - 1 WHERE ordem_exibicao > :oe AND anuncio_id = :cat");
			$update_itens->bindValue(':oe',$ordem_item_del['ordem_exibicao'],PDO::PARAM_INT);
			$update_itens->bindValue(':cat',$ordem_item_del['anuncio_id'],PDO::PARAM_INT);
