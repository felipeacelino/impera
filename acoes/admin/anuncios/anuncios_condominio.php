...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar filtro (Condomínio)";
		break;
	case 'edit':
		$tit_pag_geral = "Editar filtro (Condomínio)";
		break;
	default:
		$tit_pag_geral = "Condomínio (Filtros)";
		break;
}
...
if($acao == "insert"){
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "insert" && TOKEN == $token_confirma){
		...
		if ($verificaUnicos === true) {
          ...
          $dados = array(
            'titulo' => $_POST['titulo'],
            'only_admin' => $_POST['only_admin'] != "" ? $_POST['only_admin'] : 0,
