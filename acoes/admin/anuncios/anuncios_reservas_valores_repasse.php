<?
// CONFIGURAÇÕES
$tabela = "anuncios_reservas_valores_repasse";
$tabelaReserva = "anuncios_reservas";
$order = "ORDER BY data_repasse ASC";
$paginacao = true;
$num_regs = 15;
$ordemExibicao = false;
$uploadArquivo = true;
...
switch ($acao_enviado) { 
	case 'insert':
		$tit_pag_geral = "#".$reserva['codigo']." &rsaquo; Cadastrar repasse";
		break;
	case 'edit':
		$tit_pag_geral = "#".$reserva['codigo']." &rsaquo; Editar repasse";
		break;
	default:
...
if ($acao == "view") {	
    ...
    if ($_POST['filtro_status'] != "") {
        ...
        $filtro_status = " AND status_repasse = '".$_SESSION[$pag_include]['filtros']['filtro_status']."'";	
  ...
  if (isset($_POST['filtro_data'])) {
      if ($_POST['filtro_data'] != "") {
          ...
          $filtro_data = " AND (data_repasse BETWEEN '".$dataInit."' AND '".$dataEnd."')";	
          $filtro .= $filtro_data;     
      }else{
