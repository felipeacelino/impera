...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar newsletter";
		break;
	case 'edit':
		$tit_pag_geral = "Editar newsletter";
		break;
	default:
		$tit_pag_geral = "Newsletter";
		break;
}
...
if ($acao == "view") {	
	...
	if (isset($_POST['filtro_email'])) {
	    if ($_POST['filtro_email'] != "") {
	        ...
	        $filtro_email = " AND email LIKE '%".$_SESSION[$pag_include]['filtros']['filtro_email']."%'";	
	        $filtro .= $filtro_email;       
	    }else{
	...
	$lista_exporta = $conexao->prepare("SELECT email FROM $tabela WHERE status = 1 ORDER BY email ASC");
	$lista_exporta->execute();
	$num_lista_exporta = $lista_exporta->rowCount();
	$lista_exporta = $lista_exporta->fetchAll(PDO::FETCH_COLUMN);
	$lista_exporta = $num_lista_exporta > 0 ? implode(', ', $lista_exporta) : "";
