...
switch ($acao_enviado) {
	case 'insert':
		$tit_pag_geral = "Cadastrar locatário";
		break;
	case 'edit':
		$tit_pag_geral = "Editar locatário";
		break;
	default:
		$tit_pag_geral = "Locatários";
		break;
}
...
if($acao == "insert"){
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "insert" && TOKEN == $token_confirma){
		...
		if ($verificaUnicos === true) {
          ...
          'status_atendimento' => $_POST['status_atendimento'] != "" ? $_POST['status_atendimento'] : 0
        );	
...
if($acao == "delete" && TOKEN == $token_confirma_del){
	...
	$adicionais = array(
    'locatarios_arquivos' => 'id_usuario',
    'locatarios_mensagens' => 'id_usuario'
  );
