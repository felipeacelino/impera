...
if ($item == "") {
	Tools::redireciona("".URL."admin");
}

$filtro = "WHERE item_id = ".$item['id'];
switch ($acao_enviado) { 
	case 'insert':
	$tit_pag_geral = "Fotos: ".$item['titulo']." &rsaquo; Cadastrar fotos";
	break;
	case 'edit':
	$tit_pag_geral = "Fotos: ".$item['titulo']." &rsaquo; Gerenciar fotos";
	break;
	default:
	$tit_pag_geral = "Fotos: ".$item['titulo']." &rsaquo; Gerenciar fotos";
	break;
...
$habilita_destaque = false;  // Habilita o destaque das imagens
$habilita_zoom = true;       // Habilita o zoom das imagens
$limiteQtde = 0;             // Limite de imagens (0 = ilimitado)
$maxFileSize = 8;            // Tamanho máximo da imagem em MB
$maxFileCount = 50;          // Quantidade máxima de imagens (Válido somenta quando não há limites)
...
"".URL."admin/".$pasta_modulo."/".$tabelaItem."/".TOKEN."/edit/pag_".$item['pagina'] :
"".URL."admin/".$pasta_modulo."/".$tabelaItem."/".TOKEN."/edit/pag_".$item['pagina'];
