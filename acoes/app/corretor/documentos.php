  ...
  if (!empty($_POST)) {
    ...
    if ($arquivos->Insert($dados)) {
      ...
      $dados_email = array(
        ...
        'texto' => 'O corretor <b>' . $user['nome'] . '</b> acaba de enviar um documento, entre na área administrativa para ver mais detalhes.',
        'botao' => array(
      ...
      $destinatarios = array(EMAIL_DOCS_CORRETORES);
...
if ($acao == "update") {
  ...
  if (!empty($_POST)) {
    ...
    if ($arquivos->Update($dados, "WHERE id=$idArquivo AND id_usuario=$idUser")) {
      ...
      $dados_email = array(
        'titulo' => 'Documento Recebido',
        'texto' => 'O corretor <b>' . $user['nome'] . '</b> acaba de atualizar um documento, entre na área administrativa para ver mais detalhes.',
        'botao' => array(
          'texto' => 'Acessar Administração',
          'url' => URL . 'admin'
        )
