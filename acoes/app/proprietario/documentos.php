...
if ($_POST['acao'] != 'view') {
  ...
  include_once("" . ACOES_APP_PATH . "/proprietario/restrito.php");
}
...
$imoveisUser = $arquivos->SelectSQL("SELECT id, codigo, titulo FROM anuncios WHERE id_usuario=$idUser AND tipo_user='proprietario' ORDER BY id ASC");
  ...
  if (!empty($_POST)) {
    ...
    if ($arquivos->Insert($dados)) {
      ...
      $dados_email = array(
        ...
        'texto' => 'O proprietário <b>' . $user['nome'] . '</b> acaba de enviar um documento, entre na área administrativa para ver mais detalhes.',
        'botao' => array(
      ...
      $destinatarios = array(EMAIL_DOCS_PROPRIETARIOS);
      ...
      $retorno = URL . "proprietario/documentos/cad-success";
...
if ($acao == "update") {
  ...
  if (!empty($_POST)) {
    ...
    if ($arquivos->Update($dados, "WHERE id=$idArquivo AND id_usuario=$idUser")) {
      ...
      $dados_email = array(
        'titulo' => 'Documento Recebido',
        'texto' => 'O proprietário <b>' . $user['nome'] . '</b> acaba de atualizar um documento, entre na área administrativa para ver mais detalhes.',
        'botao' => array(
          'texto' => 'Acessar Administração',
          'url' => URL . 'admin'
        )
      ...
      $retorno = URL . "proprietario/documentos/edit-success";
