...
$retorno = isset($_GET['retorno']) && $_GET['retorno'] != "" ? Tools::protege($_GET['retorno']) : URL."afiliado/inicio/success";
$urlRetorno = isset($_GET['retorno']) && $_GET['retorno'] != "" ? URL."afiliado/entrar?retorno=".$retorno : URL."afiliado/entrar";
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<div class="container">
  <div class="pg-login-wrapper">
    ...
    <!-- Formulário -->
    <form id="form-cadastro" action="<?=URL?>acoes/app/afiliado/cadastro.php" method="post" class="pg-login-form pg-login-form-cadastro form-validation">
      ...
      <div class="campo-container campo-icon">
        <input type="email" name="email" id="cd-email" maxlength="255" class="campo" placeholder="Digite seu e-mail" required data-parsley-trigger="change" data-parsley-remote="<?=URL?>acoes/app/afiliado/verifica_email.php" data-parsley-remote-options='{ "type": "POST" }' data-parsley-remote-validator="remote" data-parsley-remote-message="Este endereço de e-mail já está sendo utilizado" />
        <label class="icon" for="cd-email"><i class="las la-envelope"></i></label>
      </div>

      <div class="campo-container campo-icon">
        <input type="telefone" name="telefone" id="cd-telefone" maxlength="255" class="campo telefone" placeholder="Digite seu telefone" data-parsley-trigger="change" data-parsley-pattern="(\([1-9]{2}\))\s([9]{1})?([0-9]{4})-([0-9]{4})" data-parsley-pattern-message="Digite um número de telefone válido." required />
      ...
      <div class="btn-container">
        <input type="hidden" name="facebook_id" id="facebook_id_cad"
          data-url="<?=URL?>acoes/app/afiliado/verifica_facebook.php">
