...
$retorno = isset($_GET['retorno']) && $_GET['retorno'] != "" ? Tools::protege($_GET['retorno']) : URL."corretor/inicio/success";
$urlRetorno = isset($_GET['retorno']) && $_GET['retorno'] != "" ? URL."corretor/entrar?retorno=".$retorno : URL."corretor/entrar";
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<div class="container">
  <div class="pg-login-wrapper">
    ...
    <!-- Formulário -->
    <form id="form-cadastro" action="<?=URL?>acoes/app/corretor/cadastro.php" method="post" class="pg-login-form pg-login-form-cadastro form-validation">
      ...
      <div class="campo-container campo-icon">
        <input type="text" inputmode="numeric" name="creci" id="cd-creci" maxlength="255" class="campo" placeholder="Informe seu CRECI" data-parsley-trigger="change" required />
        <label class="icon" for="cd-creci"><i class="las la-address-book"></i></label>
      </div>

      <div class="campo-container campo-icon">
        <input type="email" name="email" id="cd-email" maxlength="255" class="campo" placeholder="Digite seu e-mail" required data-parsley-trigger="change" data-parsley-remote="<?=URL?>acoes/app/corretor/verifica_email.php" data-parsley-remote-options='{ "type": "POST", "data": { "tipo": "corretor" } }' data-parsley-remote-validator="remote" data-parsley-remote-message="Este endereço de e-mail já está sendo utilizado" />
